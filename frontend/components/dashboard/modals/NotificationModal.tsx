...
export function NotificationModal({ onClose }: NotificationModalProps) {
  ...
  const unreadNotifications = data?.notifications?.items.filter((n: any) => !n.read) || [];
  const currentNotification = unreadNotifications[0];

  const mutation = useMutation({
    mutationFn: async (id: string) => {
      await apiFetchClient(`journey/notifications/${id}`, {
        method: 'PATCH',
  ...
  return (
    <div className="bg-canvas-surface border border-tactical-border rounded-xl p-8 w-full max-w-lg shadow-xl animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <button
          onClick={() => mutation.mutate(currentNotification.id)}
      ...
      <h2 className="font-display text-lg font-semibold text-pearl-text mb-3">
        {currentNotification.title || "System Notification"}
      </h2>

      <p className="font-sans text-sm text-muted-text leading-relaxed mb-8 whitespace-pre-wrap">
